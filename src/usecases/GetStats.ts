...
export class GetStats {
  async execute(dto: InputDto): Promise<OutputDto> {
    ...
    const workoutPlan = await prisma.workoutPlan.findFirst({
      ...
      include: {
        workoutDays: {
          include: { userWorkoutSessions: true },
        },
      },
    });
    ...
    const sessions = await prisma.userWorkoutSession.findMany({
      where: {
        workoutDay: {
    ...
    sessions.forEach((session) => {
      const dateKey = dayjs.utc(session.startAt).format("YYYY-MM-DD");
    ...
    const totalTimeInSeconds = completedSessions.reduce((total, session) => {
      const start = dayjs.utc(session.startAt);
  ...
  private async calculateStreak(
    ...
    const allSessions = await prisma.userWorkoutSession.findMany({
      where: {
    ...
    const completedDates = new Set(
      allSessions.map((s) => dayjs.utc(s.startAt).format("YYYY-MM-DD"))
    );
