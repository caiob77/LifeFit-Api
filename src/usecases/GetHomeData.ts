...
export class GetHomeData {
  async execute(dto: InputDto): Promise<OutputDto> {
    ...
    const weekSessions = await prisma.userWorkoutSession.findMany({
      where: {
        workoutDay: {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const daySessions = weekSessions.filter(
        (s) => dayjs.utc(s.startAt).format("YYYY-MM-DD") === dateKey
      );
        ...
        todayWorkoutDay && workoutPlan
          ? {
              ...
              estimatedDurationInSeconds:
                todayWorkoutDay.estimatedDurationSeconds,
